...
function generateAssistantReply(input: string): string {
  ...
  if (q.includes('graph') || q.includes('path') || q.includes('attack'))
    return 'Primary path: External IP → NGINX Exploit → Container Escape → Credential Dump → Lateral Movement → RDS. MiMo correlated 6 stages with 94% confidence.'
  ...
  if (q.includes('parallel') || q.includes('agent') || q.includes('analysis'))
    return 'Launching 4-agent parallel pipeline: Threat → CVE → Forensics → Response. Estimated 2.1s on MiMo-V2.5-Reasoner. Monitor Multi-Agent Workflow for live status.'
  return `Query processed by MiMo-V2.5-Reasoner. Context window: 1M tokens. Correlation updated for: "${input}".`
}
      ...
      <aside className="w-full max-w-xl h-full bg-[#0a0f1a] border-l border-white/10 flex flex-col shadow-2xl">
        <div className="px-6 py-5 border-b border-white/10 flex items-center justify-between">
          <div>
            ...
            <div className="text-lg font-bold mt-1">MiMo Agent Terminal</div>
          </div>
          <button
            type="button"
            onClick={onClose}
    ...
    <div className="fixed inset-0 z-[90] flex justify-end">
      ...
      <aside className="w-full max-w-md h-full bg-[#0a0f1a] border-l border-white/10 flex flex-col">
        <div className="px-6 py-5 border-b border-white/10">
          ...
          <div className="text-lg font-bold mt-1">MiMo-V2.5 Chat</div>
        </div>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {busy && (
            <div className="text-xs text-white/40 animate-pulse">MiMo reasoning...</div>
          )}
          <div ref={bottomRef} />
        </div>
...
export function useSocInteractions() {
  ...
  const [terminalLines, setTerminalLines] = useState([
    ...
    'Connected to MiMo-V2.5-Reasoner · cluster prod-soc-01',
  ])
